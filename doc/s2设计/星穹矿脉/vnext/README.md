...
前十天固定序列使用 10 分钟步进，自动采购也在每个 10 分钟块结算；玩家仅在 `00:00`、`08:00`、`16:00` 获得三个批量升级窗口，严格遵守每日最多三条消息。运行 `s2_first_ten_days.py --days 10` 会输出每日摘要，并生成 `FIRST_TEN_DAYS_TRACE.md`，记录所有手动与自动升级时点。
